...
/**
 * 自定义图片控件
 * Created by minidroid on 2017/5/6 22:07.
 * Email:460821714@qq.com
 */
public class CustomImageView extends ImageView {
    private static final ScaleType SCALE_TYPE = ScaleType.CENTER_CROP;//定义缩放类型的常量
    ...
    private final RectF mDrawableRect = new RectF();//定义内圆形的外接正方形
    private final RectF mBorderRect = new RectF();//定义外圆的外接正方形
    ...
    private int mBitmapWidth;//位图的宽度
    private int mBitmapHeight;//位图的高度
    private float mDrawableRadius;//内圆图片半径
    private float mBorderRadius;//外圆半径

    private ColorFilter mColorFilter;//定义颜色过滤器
    //初始化false;
    ...
    //定义三个参数的构造器，从我们的自定义的attr.xml中的去获取相应的属性
    public CustomImageView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        // TODO Auto-generated constructor stub
        //通过obtainStyledAttributes 获得一组值赋给 TypedArray（数组） , 这一组值来自于res/values/attrs
        // .xml中的name="CircleImageView"的declare-styleable中。
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.CircleImageView,
                defStyle, 0);
        //通过TypedArray提供的一系列方法getXXXX取得我们在xml里定义的参数值；
        // 获取边界的宽度,通过attr.XML的值

        mBorderWidth = array.getDimensionPixelSize(R.styleable.CircleImageView_border_width,
                DEFAULT_BORDER_WIDTH);
        mBorderColor = array.getColor(R.styleable.CircleImageView_border_color,
                DEFAULT_BORDER_COLOR);
        // 获取边界的颜色
        mBorderOverlay = array.getBoolean(R.styleable.CircleImageView_border_overlay,
    ...
    public CustomImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);//两个参数的构造器调用有三个参数的构造器
    }

    private void initView() {
        // TODO Auto-generated method stub
        super.setScaleType(SCALE_TYPE);////在这里ScaleType被强制设定为CENTER_CROP，就是将图片水平垂直居中，进行缩放。
        /*
		 * 因为通过测试会发现，setup方法要在构造方法之前调用，也就是说会出现一个问题就是，当我使用setImage...方法时，就还没有调用构造方法，
		 * 而构造方法里面是设置来自attr.xml中的值，所以说如果第一次调用setup方法还是默认样式，所以设置mReady和mSetupPending变量进行逻辑控制
		 * 作用就是保证第一次执行setup函数里下面代码要等在构造函数执行完毕时调用
		 * */
        mReady = true;//默认是false;
        if (mSetupPending) {
            setup();
            mSetupPending = false;
        }
    ...
    /**
     * 以下四个函数都是
     * 复写ImageView的setImageXxx()方法 ,或群图片资源
     * 注意这个函数先于构造函数调用之前调用
     *
     * @param bm
     */
    ...
    private void setup() {
        ...
        if (!mBorderOverlay) {
            //demo里始终执行
            //通过inset方法  使得图片显示的区域从mBorderRect大小上下左右内移边界的宽度形成区域，查看xml边界宽度为2dp（3px）,
            // 所以方形边长为就是160-4=156dp(234px)
    ...
    private void updateShaderMatrix() {
        ...
        // 这里不好理解 这个不等式也就是(mBitmapWidth / mDrawableRect.width()) > (mBitmapHeight / mDrawableRect
        // .height())
        //取最小的缩放比例
